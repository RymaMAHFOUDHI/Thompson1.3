...
def nfa_to_dfa(nfa):
    ...
    unmarked = [start_set]
    dfa_states = {start_set:0}; dfa_trans={}; dfa_accepts=set()
    if accept in start_set: dfa_accepts.add(start_set)
    sink = frozenset({'sink'}); sink_needed=False
    while unmarked:
        T = unmarked.pop()
        for sym in symbols:
...
def minimize_dfa(dfa):
    states = list(dfa['states']); symbols = dfa['symbols']
    accepts = set(dfa['accepts']); non_accepts = set(states)-accepts
    ...
    while W:
        ...
        for c in symbols:
            ...
            for Y in P:
                inter = Y & X; diff = Y-X
                if inter and diff:
                    newP += [frozenset(inter),frozenset(diff)]
                    if Y in W: W.remove(Y); W+=[frozenset(inter),frozenset(diff)]
    ...
    for b in new_states:
        ...
        for sym in symbols:
            ...
            for blk in new_states:
                if dest in blk: new_trans[(b,sym)]=blk; break
    # Renommage I,II,III...
    roman=["I","II","III","IV","V","VI","VII","VIII","IX","X"]
    mapping={b:roman[i] if i<len(roman) else f"X{i}" for i,b in enumerate(new_states)}
    legend={mapping[b]:"{" + ",".join(sorted(str(s) for s in b)) + "}" for b in new_states}
...
def build_nfa_graph(nfa,new_edges=None):
    ...
    for s,lst in transitions.items():
        for sym,d in lst:
            label=EPS if sym==EPS else str(sym)
            color="red" if new_edges and (s,sym,d) in new_edges else "black"
            g.edge(f"n{s}",f"n{d}",label=label,color=color)
    return g

def build_dfa_graph(dfa,minimal=False):
    mapping = getattr(dfa,'mapping',None)
    legend = getattr(dfa,'legend',None)
    if mapping is None:
        mapping={s:chr(ord('A')+i) for i,s in enumerate(dfa['states'])}
        legend={mapping[s]:"{" + ",".join(str(k) for k in s) + "}" for s in dfa['states']}
    g=graphviz.Digraph()
    g.attr(rankdir='LR', ranksep='1', nodesep='0.5')
    g.attr('node', shape='circle', fixedsize='true', width='1.2', height='1.2', fontsize='12')
    g.node('start',shape='point',width='0.1',height='0.1',fixedsize='true')
    g.edge('start',mapping[dfa['start']])
    # transitions multi-symbol
    edges={}
    for (src,sym),dest in dfa['transitions'].items():
        src_id=mapping.get(src,str(src)); dest_id=mapping.get(dest,str(dest))
        edges.setdefault((src_id,dest_id),[]).append(EPS if sym==EPS else str(sym))
    for (src,dest),syms in edges.items():
        g.edge(src,dest,label=",".join(syms))
    for s in dfa['states']:
        g.node(mapping[s],label=mapping[s],shape='doublecircle' if s in dfa['accepts'] else 'circle',width='1.2',height='1.2',fixedsize='true')
    return g,mapping,legend

# ---------------- Streamlit UI ----------------
st.set_page_config(page_title="Algorithme Thompson", layout="wide", page_icon="🧩")
...
with col_title:
    st.markdown("<h1 style='margin-top:0px;margin-bottom:0px;'>Algorithme de Thompson</h1>",unsafe_allow_html=True)
...
with colB: show_dfa = st.checkbox("Afficher le DFA",value=True)
with colC: show_min = st.checkbox("Afficher le DFA minimisé",value=False)
...
if st.session_state.steps:
    ...
    temp_nfa = {'start':st.session_state.final_nfa['start'],'accept':st.session_state.final_nfa['accept'],'transitions':step['transitions']}
    dot_nfa = build_nfa_graph(temp_nfa,step.get('new',[]))
    st.subheader("NFA actuel")
    st.graphviz_chart(dot_nfa.source)

    if show_dfa:
        dfa = nfa_to_dfa(st.session_state.final_nfa)
        st.subheader("DFA")
        gdfa,mapping,legend = build_dfa_graph(dfa)
    ...
    if show_min:
        ...
        gmin,m_mapping,m_legend = build_dfa_graph(min_dfa)
        st.graphviz_chart(gmin.source)
        leg_lines = [f'- **{lab}** = `{content}`' for lab,content in min_dfa['legend'].items()]
        st.markdown('\n'.join(leg_lines))
else:
